"""Fine-tune GSWarpNet with stronger high-reflectivity weighting (DBZ mode).

Must be run AFTER scripts/train.py has produced a base checkpoint.

The intensity weight becomes:
    w = (1 + 3·[t ≥ 35/60]) × (1 + intensity_power × t²)
At 60 dBZ this gives ~68× vs background; at 35 dBZ ~17×.
...
Options:
  --intensity-power FLOAT  Quadratic intensity multiplier (default 4.0)
  --epochs N               Max fine-tuning epochs (default 20)
  --lr FLOAT               Fine-tuning learning rate (default 1e-5)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="DBZ fine-tuning for GSWarpNet")
    parser.add_argument("--cache-dir",        required=True)
    parser.add_argument("--base-ckpt",        required=True,
                        help="Path to base checkpoint (best_model.pt from train.py)")
    parser.add_argument("--ckpt-dir",         default="./checkpoints/gswarpnet_dbz")
    parser.add_argument("--intensity-power",  type=float, default=4.0)
    ...
    model = GSWarpNet(t_in=4, t_out=12, base_channels=64, kernels_per_layer=2)
    ckpt  = torch.load(args.base_ckpt, map_location="cpu", weights_only=False)
    model.load_state_dict(ckpt["model_state_dict"])
    model = model.to(device)
    logger.info(
        f"Loaded base checkpoint (epoch={ckpt.get('epoch', '?')}, "
        f"val={ckpt.get('best_val_loss', float('nan')):.4f})"
    )
    logger.info(f"intensity_power={args.intensity_power}  lr={args.lr}")
    ...
    logger.info(f"Fine-tuning complete. Best val loss: {best_val:.4f}")
